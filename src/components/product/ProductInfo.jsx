...
const ProductInfo = ({ product }) => {
  ...
  useEffect(() => {
    dispatch(getDesignerById(product && product?.designerID));
  }, [product]);

  const { loading, supplierInfo } = useSelector(state => state.supplierInfo);
  console.log(supplierInfo);
  return (
    <>
      <div className="flex flex-col items-center justify-center mx-auto space-y-10 lg:flex-row sm:space-x-24 sm:space-y-0 w-[288px] sm:w-full my-2 lg:w-[98%]">
        <div className="flex flex-col sm:flex-row space-x-2 lg:w-[70%]">
          ...
          <div>
            <div className="flex justify-center h-[19rem] min-w-[300px] sm:w-64 sm:h-64 lg:h-[19rem] lg:w-[20%] border border-zinc-300 rounded-lg">
              <ProductImage productImage={product.image && product?.image[0]} />
              {/* <div className="flex">
                {product?.image.map((i, index) => {
                  <div className="w-16 h-16 border border-zinc-300">
                    <img
                      src={i}
                      alt=""
                      className="w-16 h-16"
          ...
          <div className="flex flex-col mx-8 my-4 space-y-2 w-[288px] sm:w-full lg:w-[80%]">
            ...
            <div className="w-[300px] sm:w-full lg:w-auto h-[0.1rem] bg-zinc-300"></div>
            <div className="flex space-x-2">
              <span className="text-zinc-400 w-40 lg:w-auto">Description:</span>
              <span className="text-zinc-600 w-36 sm:w-44 lg:w-auto">
                {product.description}
              </span>
            </div>
