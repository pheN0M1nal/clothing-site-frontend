...
const Wrapper = styled.button`
    ...
    background-color: var(
        --custom-${({ BgColor }) => BgColor || "orange-color"}
    );
    text-align: center;
    ...
    text-transform: ${(props) =>
        props.textTransform ? `${props.textTransform}` : "lowercase"};
    outline: none;
    ...
    font-weight: 500;
    min-width: ${(props) =>
        props.minWidth ? `${props.minWidth}rem` : "initial"};
    max-width: ${(props) =>
    ...
    :hover {
        box-shadow: ${({ addEffect }) =>
            addEffect ? "var(--custom-shadow)" : "null"};
    }
